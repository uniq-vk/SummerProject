...
const int SLEEP_TIME = 0.5  *1000000;
...
vector<vector<box>> box_coordinates;
...
void initialize(){
  text = "Solving for N = " + to_string(N);
  grid.resize(N, vector<int>(N, EMPTY));
  box_coordinates.resize(N, vector<box>(N));
}
void init(){
...
bool solve(vector<vector<int>> grid, int row){
  ...
  for(int col = 0; col < grid.size(); col++){
    ...
    if(row==0 && col==grid.size()-1)exit(0);
  }
  return false;
}
